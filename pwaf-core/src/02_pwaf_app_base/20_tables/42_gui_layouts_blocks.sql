...
BEGIN
	IF NOT EXISTS (
		SELECT 1 FROM pg_tables WHERE schemaname='pwaf_app_base' and tablename='gui_layouts_blocks'
	) THEN

		CREATE TABLE pwaf_app_base.gui_layouts_blocks
		(
		  ...
		  CONSTRAINT layout_block_rel_layout_pkey FOREIGN KEY (layout)
		      REFERENCES pwaf_app_base.gui_layouts (id) MATCH SIMPLE
		      ON UPDATE NO ACTION ON DELETE NO ACTION,
		...
		CREATE INDEX fki_layout_block_rel_block_pkey
		  ON pwaf_app_base.gui_layouts_blocks
		  USING btree
		  (block );
...
ALTER TABLE pwaf_app_base.gui_layouts_blocks OWNER TO pwaf;
--