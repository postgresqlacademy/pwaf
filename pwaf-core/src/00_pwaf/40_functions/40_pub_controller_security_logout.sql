--
CREATE OR REPLACE FUNCTION pwaf.pub_controller_security_logout(in_request pwaf.http_request)
...
BEGIN
	...
	-- Success
	UPDATE pwaf.auth_sessions SET user_id=NULL WHERE id=in_request.session_id;
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION pwaf.pub_controller_security_logout(pwaf.http_request) OWNER TO pwaf;

DELETE FROM pwaf.http_request_routes WHERE path = '/security/logout';
INSERT INTO pwaf.http_request_routes (id, path, controller) VALUES (402, '/security/logout','pwaf.pub_controller_security_logout');
--