--
CREATE OR REPLACE FUNCTION pwaf.pub_controller_security_login(in_request pwaf.http_request)
...
BEGIN
	...
	IF (pwaf.pub_http_request_param_get(in_request,'security_login_username') IS NOT NULL AND pwaf.pub_http_request_param_get(in_request,'security_login_password') IS NOT NULL) THEN
		...
		SELECT auth_users.id, auth_types.auth_type, auth_users.salt FROM pwaf.auth_users, pwaf.auth_types WHERE auth_types.id=auth_users.auth_type_id AND auth_users.user_name = v_username INTO v_user_id, v_auth_type, v_salt;
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION pwaf.pub_controller_security_login(pwaf.http_request) OWNER TO pwaf;

DELETE FROM pwaf.http_request_routes WHERE path = '/security/login';
INSERT INTO pwaf.http_request_routes (id, path, controller) VALUES (401, '/security/login','pwaf.pub_controller_security_login');
--