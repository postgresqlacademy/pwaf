...
BEGIN
	...
	IF ((pwaf.pub_http_request_system_param_get(in_request,'session_id') IS NOT NULL) AND (pwaf.pub_http_request_system_param_get(in_request,'Remote-Addr') IS NOT NULL)) THEN
		...
		IF v_session_id IS NULL THEN
			INSERT INTO pwaf.auth_sessions (ip_address, cookie, last_activity) VALUES (v_remote_addr,v_cookie, now()) RETURNING id INTO v_session_id;
		ELSE
			UPDATE pwaf.auth_sessions SET last_activity=now() WHERE id=v_session_id;
		END IF;
