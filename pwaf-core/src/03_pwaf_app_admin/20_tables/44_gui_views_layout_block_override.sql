...
BEGIN
	IF NOT EXISTS (
		SELECT 1 FROM pg_tables WHERE schemaname='pwaf_app_admin' and tablename='gui_views_layout_block_override'
	) THEN

		CREATE TABLE pwaf_app_admin.gui_views_layout_block_override
		(
			...
			CONSTRAINT view_layout_block_override_view_fkey FOREIGN KEY (view)
			  REFERENCES pwaf_app_admin.gui_views (id) MATCH SIMPLE
			  ON UPDATE NO ACTION ON DELETE NO ACTION,
		...
		CREATE INDEX fki_view_layout_block_override_block_fkey
		  ON pwaf_app_admin.gui_views_layout_block_override
		  USING btree
		  (block );
...
ALTER TABLE pwaf_app_admin.gui_views_layout_block_override OWNER TO pwaf;
--